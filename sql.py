...
SQL = os.path.expanduser('~/.khl/osu.db')
...
class osusql():
    ...
    def get_name_mod(self, uid):
        try:
            result = self.con().execute(f'select osuname, osumod from userinfo where khlid = {uid}').fetchall()
            return result
        except Exception as e:
            print(e)
    ...
    def get_id_mod(self, uid):
        try:
            result = self.con().execute(f'select osuid, osumod from userinfo where khlid = {uid}').fetchall()
            return result
        except Exception as e:
            print(e)
    ...
    def update_mode(self, uid, osumod):
        try:
            con = self.con()
            con.execute(f'update userinfo set osumod = {osumod} where khlid = {uid}')
            con.commit()
            return True
        except Exception as e:
            print(e)
    ...
    def delete_user(self, uid):
        try:
            con = self.con()
            con.execute(f'delete from userinfo where khlid = {uid}')
            con.commit()
            return True
        except Exception as e:
            print(e)
